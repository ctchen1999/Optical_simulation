...
as = ones(N,1);
epsr = ones(N,1); % eps_relative
mur= ones(N,1);   % µ_relative
sigma = zeros(N,1);
...
%You can define epsr(i), mur(i) (relative permittivity and permea bility)
...
ae = ae./epsr; % eps = eps0 * epsr | permitivity
am = am./mur;  % µ = µ0 * µr | permeativity
ae = ae./(1+dt*(sigma./epsr)/(2*eps0));
as = (1-dt*(sigma./epsr)/(2*eps0))./(1+dt*(sigma./epsr)/(2*eps0)); % conductivity
...
Ey = zeros(N,1);
figure(2);
set(gcf,'doublebuffer','off'); %set double buffering on for smoother graphics
plot(Ey);
grid on;
